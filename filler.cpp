...
animation filler::fillCustomDFS(PNG& img, int x, int y, double tolerance, int frameFreq)
{
    HSLAPixel center = *img.getPixel(x,y);
    customColorPicker a(img, tolerance, center);
    return fill<Stack>(img,x,y,a,tolerance,frameFreq);
}
...
animation filler::fillCustomBFS(PNG& img, int x, int y, double tolerance, int frameFreq)
{
    HSLAPixel center = *img.getPixel(x,y);
    customColorPicker a(img, tolerance, center);
    return fill<Queue>(img,x,y,a,tolerance,frameFreq);
                       ...
                       double tolerance, int frameFreq)
{
     ...
     */
     ...
     visited.resize(width + 1);
     for(int i=0; i < width; i++){
         visited[i].resize(height + 1);
     }
     for(int i=0; i < width; i++){
     ...
     OrderingStructure<int> structureX;
     OrderingStructure<int> structureY;
     structureX.add(x);
     structureY.add(y);
     HSLAPixel ctrPt = *img.getPixel(x,y);
     *img.getPixel(x,y) = fillColor(x,y);
     ...
     while(!structureX.isEmpty() && !structureY.isEmpty()){
         int currX = structureX.remove();
         int currY = structureY.remove();
         for(int i = 0; i < 8; i++){
             ...
             if(ctrPt.dist(*img.getPixel(nextX,nextY)) <= tolerance){
                 structureX.add(nextX);
                 structureY.add(nextY);
                 HSLAPixel colour = fillColor(nextX,nextY);
                 *img.getPixel(nextX,nextY) = colour;
